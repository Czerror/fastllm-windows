...
├── ftllm/                  # Python 模块目录
│   ├── pyfastllm.*.pyd    # Python 绑定模块
│   ├── fastllm_tools.dll  # Python 工具库
...
```cmd
# Python CLI
ftllm run D:\Models\Qwen2-7B-Instruct --device cuda

# 原生程序
main.exe -p D:\Models\Qwen2-7B-Instruct --device cuda
```
