...
```
  ...
  --nt <值>              输入<think>归一化为特殊token，稳定KV缓存命中 (true/false)
  --hide_input           不显示请求信息
...
- `--nt` 控制的是“输入消息里出现 <think> 时是否做归一化”，用于避免下一轮回传导致 token 序列变化，从而影响 KV 前缀缓存命中。
...
# Qwen3 / Qwen3-Next（建议开启输入 <think> 归一化以提升多轮对话 KV 缓存命中）
ftllm serve D:\Models\Qwen3 --port 8080 --device cuda --nt true
```

提示：如果你使用的是 Qwen3 系列并且客户端会把输出中的 `<think>` 原样回传到下一轮 messages，建议显式加 `--nt true`。
