...
class FastLLmCompletion:
    def __init__(self, model_name, model, think, hide_input, normalize_think: bool = False):
        self.model_name = model_name
        self.model = model
        self.init_fast_llm_model()
        self.think = think
        self.hide_input = hide_input
        self.normalize_think = normalize_think
    ...
    def _normalize_messages_for_cache(self, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        # 说明：部分模型（如 Qwen3 系列）会用特殊 token 表示 <think> 标签。
        # 我们在流式输出时把 "[unused16]/[unused17]" 显示为 "<think></think>"，
        # 但如果客户端把这些内容原样回传到下一轮 messages，就会导致 prompt token 序列改变，
        # 从而让 PastKVCacheManager 的前缀命中失效，表现为每轮都 Long Prefill。
        # 这里把回传的可见标签转换回模型的特殊 token 文本表示，尽量保证 token 序列稳定。
        normalized: List[Dict[str, str]] = []
        for msg in messages:
            role = msg.get("role", "")
            content = msg.get("content", "")
            if isinstance(content, str) and content:
                content = content.replace("<think>", "[unused16]").replace("</think>", "[unused17]")
    ...
    async def create_chat_completion(
        ...
        try:
            ...
            for m in request.messages:
                messages, _ = self._parse_chat_message_content(m["role"], m["content"])
        ...
        # 可选：由主程序决定是否开启（例如 Qwen3 系列可开启以稳定 KV 前缀命中）
        if self.normalize_think:
            messages = self._normalize_messages_for_cache(messages)
