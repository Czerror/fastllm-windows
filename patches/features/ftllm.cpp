...
/**
 * Python 后端通过 ctypes.CDLL 加载 ftllm\fastllm_tools.dll，
 * 但 Windows 解析该 DLL 的依赖（CUDA 运行库、VC++ 运行库等）时
 * 不会自动搜索 ftllm/ 目录。这里把 exeDir\bin 前置到当前进程 PATH，
 * 使 Python 子进程继承后能正确解析全部运行库（方法 2，不动上游源码）。
 */
static void prependBinDirToPath(const std::string& exeDir) {
    const std::string binDir = exeDir + "\\bin";
    if (!isDirectory(binDir)) return;

    const DWORD pathLen = GetEnvironmentVariableA("PATH", nullptr, 0);
    std::string oldPath;
    if (pathLen > 0) {
        oldPath.resize(static_cast<size_t>(pathLen));
        GetEnvironmentVariableA("PATH", oldPath.data(), pathLen);
        if (!oldPath.empty() && oldPath.back() == '\0') oldPath.pop_back();
    }

    const std::string newPath = binDir + ";" + oldPath;
