...
int main(int argc, char **argv) {
    ...
    if (!fastllm::FileExists(config.path)) {
        printf(reinterpret_cast<const char*>(u8"模型文件 %s 不存在！\n"), config.path.c_str());
        exit(0);
    }
    ...
    printf(reinterpret_cast<const char*>(u8"欢迎使用 %s 模型. 输入内容对话，reset清空历史记录，stop退出程序.\n"), model->model_type.c_str());

    while (true) {
        printf(reinterpret_cast<const char*>(u8"用户: "));
        std::string input;
